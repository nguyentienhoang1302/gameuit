...
EnemyTankStartingState::EnemyTankStartingState(EnemyData *pData, int s)
{
	this->pData = pData;
	getSprite();
	iFrames = 0;
...
EnemyTankStartingState::~EnemyTankStartingState()
{

}

void EnemyTankStartingState::getSprite()
{
	if (pData->HP >30)
...
void EnemyTankStartingState::onUpdate()
{
		...
		if (pData->iCurrentArr == 13)
			...
			if (pData->iCurrentArr == 14)
				...
				if (pData->iCurrentArr == 15)
					...
					index = 9;
		transition(new EnemyTankShootingState(pData, index));
	}
