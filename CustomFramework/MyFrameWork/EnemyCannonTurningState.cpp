...
EnemyCannonTurningState::EnemyCannonTurningState(EnemyData* pData)
{
	this->pData = pData;
...
EnemyCannonTurningState::~EnemyCannonTurningState()
{

}

void EnemyCannonTurningState::onUpdate()
{
	this->getIndex();
	if ((count == 0 || count == 25 || count == 50) && directPlayerX < pData->x && directPlayerY < pData->y)
	{
		pData->bulletsVector.push_back(new EnemyWhiteBullet(bulletX, bulletY, true, angle));
	}
...
void EnemyCannonTurningState::onDead()
{
	transition(new EnemyCannonDeadState(pData));
}

void EnemyCannonTurningState::getIndex()
{
	...
	if (tanalpha < 0.24f)
	{
		this->pData->iCurrentArr = EnemyCannonData::TURN0;
		angle = M_PI;
	...
	else
	{
		if (tanalpha < 1.45f)
		{
			this->pData->iCurrentArr = EnemyCannonData::TURN30;
			angle = M_PI * 7 / 6;
			bulletX = pData->x - pData->body.width / 2;
		...
		{
			this->pData->iCurrentArr = EnemyCannonData::TURN60;
			angle = M_PI * 4 / 3;
