...
EnemyInkJumpingState::EnemyInkJumpingState(EnemyData* pData)
{
	this -> pData = pData;
...
void EnemyInkJumpingState:: onUpdate()
{
...
void EnemyInkJumpingState::onCollision(RectF rect)
{
	float vx = pData -> vx;
	float vy = pData -> vy;
	...
	if( vx > 0.0f)
	{
		if(vy > 0.0f)
		{
			...
			if( vy * px > vx * py )
			{
				...
				transition(new EnemyInkStandingState( pData ));
