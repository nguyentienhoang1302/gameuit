...
EnemySniperTurningState::EnemySniperTurningState(EnemyData* pData)
{
	this->pData = pData;
...
void EnemySniperTurningState::onUpdate()
{
	...
	if (count == 80)
	{
		...
		{
		case 0:
			pData->bulletsVector.push_back(new EnemyWhiteBullet(pData->x + pData->body.width / 2 + 6, pData->y - pData->body.height * 28 / 32, true, 0));
			break;
		case 1:
			pData->bulletsVector.push_back(new EnemyWhiteBullet(pData->x + pData->body.width / 2 + 5, pData->y - pData->body.height * 13 / 32 + 1, true, fAngle));
			break;
		case 2:
			pData->bulletsVector.push_back(new EnemyWhiteBullet(pData->x - pData->body.width / 2 - 5, pData->y - pData->body.height * 13 / 32 + 1, true, fAngle));
			break;
		case 3:
			if (pData->stage == 3)
			{
				if (fAngle > M_PI - M_PI / 24)
					...
					fAngle = M_PI - M_PI /12;
				pData->bulletsVector.push_back(new EnemyWhiteBullet(pData->x - pData->body.width / 2 - 5, pData->y - pData->body.height * 28 / 32, true, fAngle));
			}
			else
			pData->bulletsVector.push_back(new EnemyWhiteBullet(pData->x - pData->body.width / 2 - 5, pData->y - pData->body.height * 28 / 32, true, M_PI));
			break;
		case 4:
			pData->bulletsVector.push_back(new EnemyWhiteBullet(pData->x - pData->body.width / 2 - 3, pData->y - pData->body.height - 8, true, fAngle));
			break;
		case 5:
			pData->bulletsVector.push_back(new EnemyWhiteBullet(pData->x + pData->body.width / 2 + 4, pData->y - pData->body.height - 8, true, fAngle));
			break;
		}
