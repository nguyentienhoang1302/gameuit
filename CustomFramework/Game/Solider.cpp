...
Solider::Solider()
{
	animations[STANDING] = new Animation(SOLIDER, 0, 2);
	animations[RUNNING] = new Animation(SOLIDER, 3, 6);
	type = ENEMY_SOLIDER;
...
void Solider::DetectCurGround(std::unordered_set<Rect*> grounds)
{
	for (auto g : grounds)
	{
...
void Solider::UpdateDistance(float dt)
{
...
void Solider::ChangeState(State StateName)
{
	switch (StateName)
	{
	case STANDING:
