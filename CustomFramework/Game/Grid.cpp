...
void Grid::CreateGridFile(int level)
{
	...
	if (ifile.fail())
	{
		...
		char fileMapName[30];
		sprintf_s(fileMapName, "Resources\\Maps\\matrix%d.txt", level);
		ifile.open(fileMapName);

		int numSetTiles, tileRows, tileCols, mapRows, mapCols;
		...
		sprintf_s(objectsFileName, "Resources\\Maps\\objects%d.txt", level);
